# first division routine is called that returns a qoutient and remainder
# if no remainder routine exits, if remainder available in modulo register
# subroutine fraction1 is entered . The remainder from previous division
# is multiplied by 1000 and inserted as the new dividend. This is again
# divided by divisor and the new qoutient is stored in fraction register.
# the fist qoutient is integer part and the 2nd qoutient is fraction part
# stored in result_lo and fraction registers respectievly. The decimal point
# can be placed by the printer function between these values while
# printing,tested on ch32v003 and found working
...
fraction1:
	call soft_mul	 # result in result_lo_m and result_hi_m
	la t0,result_lo_m
	lw a1,0(t0)
...
div32df1:
	...
	la t0,fraction		# let t0 point to result_lo
	sw a3,0(t0)		# store result of division in address  pointed by t0
	la t0,modulo		# point to address of modulo
	sw a5,0(t0)		# a5 has remainder of division 
	j here
