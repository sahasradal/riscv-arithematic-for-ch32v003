...
M14: .word 0
M15: .word 0
result_lo_mul: .word 0
result_hi_mul: .word 0
result_lo_div: .word 0
result_hi_div: .word 0
modulo:    .word 0
...
lw a1,0(t0)
li a2,1000
call int_division	# integer in result_lo_div , fraction in result_hi_div
la t0,result_lo_div	# integer
lw x7,0(t0)
la t0,M5		# M5
sw x7,0(t0)
la t0,modulo		# remainder modulo
lw x7,0(t0)
la t0,M6		# M6	
sw x7,0(t0)
...
lw x11,0(t0)
add x7,x7,x11
la t0,M9		# M1+M5+M7 all int in M9
sw x7,0(t0)
...
lw x11,0(t0)
add x7,x7,x11
la t0,M10		# M4+M6+M8 all fraction in M10
sw x7,0(t0)
...
lw x11,0(t0)
add x7,x7,x11
la t0,M12		# M6+M8+M11 all fraction = M11
sw x7,0(t0)
